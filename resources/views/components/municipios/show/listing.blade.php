...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden mb-12">
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
                <tr>
                    <th scope="col" class="px-6 py-4 font-bold text-indigo-900">CNPJ / Data Abertura</th>
                    <th scope="col" class="px-6 py-4 font-bold text-indigo-900">Empresa</th>
                    <th scope="col" class="px-6 py-4 font-bold text-indigo-900 hidden md:table-cell">Localização</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($empresas as $estabelecimento)
                    ...
                    <tr class="hover:bg-indigo-50/40 transition-colors group">
                        ...
                        {{-- Coluna 3: Endereço (Oculto no mobile) --}}
                        <td class="px-6 py-4 hidden md:table-cell">
                            ...
                            <div class="text-xs text-gray-500 mt-1">
                                CEP {{ substr($estabelecimento->cep, 0, 5) }}-{{ substr($estabelecimento->cep, 5, 3) }}
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center text-gray-400">
                                ...
                                <p class="text-sm mt-1">Não encontramos registros ativos para este município no momento.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    @if($empresas->hasPages())
        <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
            {{ $empresas->links() }} {{-- O Laravel renderiza automaticamente o estilo Tailwind se configurado --}}
        </div>
    @endif
</div>